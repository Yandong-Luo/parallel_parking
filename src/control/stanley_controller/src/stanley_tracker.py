...
class Stanley(object):
    ...
    def get_gem_pose(self):
        ...
        odom_rob_pose.pose = model_state.pose
        ...
        x = map_vehicle_pos.pose.position.x
        y = map_vehicle_pos.pose.position.y
        ...
        current_vx = model_state.twist.linear.x
        current_vy = model_state.twist.linear.y  # y方向速度分量
        ...
        while not rospy.is_shutdown():
            ...
            for i in range(len(self.path_points_x)):
                self.dist_arr[i] = self.dist(
                    (self.path_points_x[i], self.path_points_y[i]), (self.vehicle_x, self.vehicle_y)
                )
            ...
            for idx in goal_arr:
                v1 = [
                    self.path_points_x[idx] - self.vehicle_x,
                    self.path_points_y[idx] - self.vehicle_y,
                ]
                v2 = [np.cos(self.vehicle_heading), np.sin(self.vehicle_heading)]
            ...
            if self.goal == -1:
                ...
                if self.goal >= len(self.path_points_x) - 3:
                    ...
                    target_orientation = self.path_points_yaw[self.goal]
                    # target_orientation = np.arctan2(target_y-self.vehicle_y,target_x-self.vehicle_x)

                    result = self.pid_speed.update(currentPose=[self.vehicle_x,self.vehicle_y,self.vehicle_heading,self.speed],\
                    referencePose=[target_x,target_y,target_orientation,self.desired_speed])

                    self.ackermann_msg.speed = 2*result[0]
                    self.ackermann_msg.steering_angle = result[1]
                    self.ackermann_pub.publish(self.ackermann_msg)
            ...
            #     print("当前target序号",target_point_idx)
            #     self.desired_speed = 3
            #     result = self.pid_speed.update(currentPose=[self.vehicle_x,self.vehicle_y,self.vehicle_heading,self.speed],\
            #     referencePose=[self.target_path_points_x[target_point_idx],self.target_path_points_y[target_point_idx],self.target_path_points_yaw[target_point_idx],self.desired_speed])

            #     self.ackermann_msg.speed = result[0]
            #     self.ackermann_msg.steering_angle = result[1]
            #     self.ackermann_pub.publish(self.ackermann_msg)
