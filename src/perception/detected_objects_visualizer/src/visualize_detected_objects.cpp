...
void VisualizeDetectedObjects::DetectedObjectsCallback(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  cubeBox_markers = ObjectsToCubeBoxes(in_objects);

  visualization_markers.markers.insert(visualization_markers.markers.end(),
                                       cubeBox_markers.markers.begin(), cubeBox_markers.markers.end());
...
VisualizeDetectedObjects::ObjectsToCubeBoxes(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  // 不同label对应不同颜色的框
  std::vector<int> rgb_r={1,0,0};
  std::vector<int> rgb_g={0,1,0};
  std::vector<int> rgb_b={0,0,1};
  ...
  for(auto const &object : in_objects.objects){
    if (IsObjectValid(object) &&
        ...
        (object.dimensions.x + object.dimensions.y + object.dimensions.z) < object_max_linear_size_)
      {
        ...
        box.scale.x = 0.1; // 线粗细
        box.color.a = 1.0; // 透明度
        int index = m_map[object.label];

        box.color.r = rgb_r[index];
        box.color.g = rgb_g[index];
        box.color.b = rgb_b[index];

        box.points.clear();
        
        // 根据长方体端点绘制长方体
        std::vector<geometry_msgs::Point> face_1={object.corners[0],object.corners[1],object.corners[1], object.corners[2],object.corners[2], object.corners[3],object.corners[3],object.corners[0]};
        std::vector<geometry_msgs::Point> face_2={object.corners[0],object.corners[4],object.corners[4],object.corners[5],object.corners[5],object.corners[1]};
        std::vector<geometry_msgs::Point> face_3={object.corners[3],object.corners[7],object.corners[7],object.corners[6],object.corners[6],object.corners[2]};
        std::vector<geometry_msgs::Point> face_4={object.corners[7],object.corners[4],object.corners[5],object.corners[6]};

        std::vector<geometry_msgs::Point> sum;
        sum.insert(sum.end(),face_1.begin(),face_1.end());
        sum.insert(sum.end(),face_2.begin(),face_2.end());
        sum.insert(sum.end(),face_3.begin(),face_3.end());
        sum.insert(sum.end(),face_4.begin(),face_4.end());
