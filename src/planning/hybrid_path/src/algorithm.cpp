...
float aStar(Node2D& start, Node2D& goal, Node2D* nodes2D, Constants::mapInfo m_mapInfo, CollisionDetection& configurationSpace, Visualize& visualization);
//计算start到目标点goal的启发式代价(即：cost-to-go)
void updateH(Node3D& start, const Node3D& goal, Node2D* nodes2D, float* dubinsLookup, Constants::mapInfo m_mapInfo, CollisionDetection& configurationSpace, Visualize& visualization);
//Dubins shot的路径
...
Node3D* Algorithm::hybridAStar(Node3D& start,
  ...
  updateH(start, goal, nodes2D, dubinsLookup, m_mapInfo, configurationSpace, visualization);
  // mark start as open
  start.open();//将start加入open 集合: 1)将点标记为open
  // push on priority queue aka open list
  O.push(&start);// 2) 加入集合
  iPred = start.setIdx(m_mapInfo.width, m_mapInfo.height); //计算索引位置
  nodes3D[iPred] = start;
  ...
  while (!O.empty()) {
    ...
    nPred = O.top();
    // set index
    iPred = nPred->setIdx(m_mapInfo.width, m_mapInfo.height);//获取该点在nodes3D的索引 (前缀i表示index, n表示node)
    iterations++;//记录迭代次数
    ...
    // EXPANSION OF NODE
      ...
      // CONTINUE WITH SEARCH
        ...
        for (int i = 0; i < dir; i++) {//每个方向都搜索
          ...
          // 设置节点遍历标识
          iSucc = nSucc->setIdx(m_mapInfo.width, m_mapInfo.height);//索引值
          ...
          if (nSucc->isOnGrid(m_mapInfo.width, m_mapInfo.height) && configurationSpace.isTraversable(nSucc)) {
            ...
            if (!nodes3D[iSucc].isClosed() || iPred == iSucc) {
              ...
              if (!nodes3D[iSucc].isOpen() || newG < nodes3D[iSucc].getG() || iPred == iSucc) {
                ...
                updateH(*nSucc, goal, nodes2D, dubinsLookup, m_mapInfo, configurationSpace, visualization);
...
float aStar(Node2D& start,
  ...
  O.push(&start);
  iPred = start.setIdx(m_mapInfo.width);
  nodes2D[iPred] = start;
  ...
  while (!O.empty()) {
    ...
    // set index
    iPred = nPred->setIdx(m_mapInfo.width);//相应的index
    ...
    // EXPANSION OF NODE
      ...
      // CONTINUE WITH SEARCH
        ...
        for (int i = 0; i < Node2D::dir; i++) {//A*算法是8个方向：4个正方向和4个45度的方向
          ...
          // set index of the successor
          iSucc = nSucc->setIdx(m_mapInfo.width);

          // ensure successor is on grid ROW MAJOR
          // ensure successor is not blocked by obstacle
          // ensure successor is not on closed list
          // 约束性检查：在有效网格范围内、且不是障碍、没有扩展过
          if (nSucc->isOnGrid(m_mapInfo.width, m_mapInfo.height) &&  configurationSpace.isTraversable(nSucc) && !nodes2D[iSucc].isClosed()) {
...
void  updateH(Node3D& start, const Node3D& goal, Node2D* nodes2D, float* dubinsLookup, Constants::mapInfo m_mapInfo,
 CollisionDetection& configurationSpace, Visualize& visualization) {
  ...
  if (Constants::dubins) {
    ...
    //    if (Constants::dubinsLookup && uX < Constants::dubinsm_mapInfo.width - 1 && uY < Constants::dubinsm_mapInfo.width - 1) {
    ...
    //      dubinsCost = dubinsLookup[uX * Constants::dubinsm_mapInfo.width * Constants::headings * Constants::headings
  ...
  if (Constants::twoD && !nodes2D[(int)start.getY() * m_mapInfo.width + (int)start.getX()].isDiscovered()) {
    //    ros::Time t0 = ros::Time::now();
    ...
    nodes2D[(int)start.getY() * m_mapInfo.width + (int)start.getX()].setG(
      //调用A*算法，返回cost-so-far, 并在2D网格中设置相应的代价值
      aStar(goal2d, start2d, nodes2D, m_mapInfo, configurationSpace, visualization)
      );
  }
  ...
  if (Constants::twoD) {
    ...
    twoDCost = nodes2D[(int)start.getY() * m_mapInfo.width + (int)start.getX()].getG() - twoDoffset;
    //getG()返回A*的启发式代价，twoDoffset为 start与goal各自相对自身所在2D网格的偏移量的欧氏距离

  }
  // std::cout<<"max cost:"<<std::max(reedsSheppCost, std::max(dubinsCost, twoDCost))<<std::endl;
