...
void Visualize::publishNode3DPose(Node3D& node) {
  //将一个node的信息转换为ROS的geometry_msgs并通过publisher进行广播
  geometry_msgs::PoseStamped pose;
  pose.header.frame_id = "path";
  pose.header.stamp = ros::Time::now();
  pose.header.seq = 0;
  pose.pose.position.x = node.getX() * m_mapInfo.cellSize + m_mapInfo.origin_x;
  pose.pose.position.y = node.getY() * m_mapInfo.cellSize + m_mapInfo.origin_y;
...
//###################################################
// 这里的位姿包括之前在队列poses3D中的位姿
void Visualize::publishNode3DPoses(Node3D& node) {
  geometry_msgs::Pose pose;
  pose.position.x = node.getX() * m_mapInfo.cellSize + m_mapInfo.origin_x;
  pose.position.y = node.getY() * m_mapInfo.cellSize + m_mapInfo.origin_y;
...
void Visualize::publishNode2DPose(Node2D& node) {
  ...
  pose.header.stamp = ros::Time::now();
  pose.header.seq = 0;
  pose.pose.position.x = (node.getX() + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_x;
  pose.pose.position.y = (node.getY() + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_y;
...
void Visualize::publishNode2DPoses(Node2D& node) {
  if (node.isDiscovered()) {
    geometry_msgs::Pose pose;
    pose.position.x = (node.getX() + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_x;
    pose.position.y = (node.getY() + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_y;
...
void Visualize::publishNode3DCosts(Node3D* nodes, int width, int height, int depth) {
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    if (values[i] != 1000) {
     ...
     //填充costCube结构体，存入costCubes
      costCube.header.frame_id = "path";
      costCube.header.stamp = ros::Time::now();
      costCube.id = i;
      costCube.type = visualization_msgs::Marker::CUBE;
      values[i] = (values[i] - min) / (max - min);
      costCube.scale.x = m_mapInfo.cellSize;
      costCube.scale.y = m_mapInfo.cellSize;
      costCube.scale.z = 0.1;
      costCube.color.a = 0.5;
      heatMapGradient.getColorAtValue(values[i], red, green, blue);//转成颜色值
      costCube.color.r = red;
      costCube.color.g = green;
      costCube.color.b = blue;
      // center in cell +0.5
      costCube.pose.position.x = (i % width + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_x;
      costCube.pose.position.y = ((i / width) % height + 0.5) * m_mapInfo.cellSize + m_mapInfo.origin_y;
...
void Visualize::publishNode2DCosts(Node2D* nodes, int width, int height) {
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    if (nodes[i].isDiscovered()) {
      ...
      costCube.scale.z = 0.1;
      costCube.color.a = 0.5;
      heatMapGradient.getColorAtValue(values[i], red, green, blue);//转化成颜色
      costCube.color.r = red;
      costCube.color.g = green;
      costCube.color.b = blue;
      // center in cell +0.5
