...
Planner::Planner() {
	...
	subGoal = n.subscribe("/goal_pose", 1, &Planner::setGoal, this);
	subStart = n.subscribe("/start_pose", 1, &Planner::setStart, this);

	subReplan = n.subscribe("/replanning",1,&Planner::setRePlan, this);
	subGetFinal = n.subscribe("/getFinalPath",1,&Planner::getPath_callBack, this);
};

void Planner::setRePlan(const std_msgs::Bool replan){
  this->replan = replan.data;
  std::cout<<this->replan<<std::endl;
}

void Planner::getPath_callBack(const std_msgs::Bool getPath){
    std::cout<<"收到发出final path的请求"<<std::endl;
    smoothedPath.publishFinalPath();
}
...
void Planner::setStart(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& initial) {
	...
	float x = initial->pose.pose.position.x / Constants::cellSize + Constants::offset_x;
	float y = initial->pose.pose.position.y / Constants::cellSize + Constants::offset_y;
...
void Planner::setGoal(const geometry_msgs::PoseStamped::ConstPtr& end) {
	...
	float x = end->pose.position.x / Constants::cellSize + Constants::offset_x;
	float y = end->pose.position.y / Constants::cellSize + Constants::offset_y;
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    // retrieving goal position
    float x = goal.pose.position.x / Constants::cellSize + Constants::offset_x;
    float y = goal.pose.position.y / Constants::cellSize + Constants::offset_y;
    ...
    x = start.pose.pose.position.x / Constants::cellSize + Constants::offset_x;
    y = start.pose.pose.position.y / Constants::cellSize + Constants::offset_y;
